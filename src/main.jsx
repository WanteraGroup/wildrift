...
const itemIconIds={'Fiendhunter Bolts':2512,'Yun Tal Wildarrows':3036,'Infinity Edge':3031,'Mortal Reminder':3033,'Guardian Angel':3026,'Gluttonous Greaves':3172,'Luden’s Echo':6655,'Infinity Orb':2158,'Rabadon’s Deathcap':3089,'Void Staff':3135,'Zhonya’s Hourglass':3157,'Mana Boots':3175,'Sunfire Aegis':3068,'Thornmail':3075,'Amaranth’s Twinguard':2362,'Force of Nature':4401,'Randuin’s Omen':3143,'Plated Steelcaps':3047}
const matchSeed=[
...
function itemImage(item){return itemIconIds[item]?communityDragonBase+'/assets/items/icons2d/'+itemIconIds[item]+'.png':''}
    ...
    <main className="container">
      ...
      {tab==='build'&&<section className="layout">
        <aside className="panel"><div className="panelTitle"><h2>01 / MATCH SETUP</h2><span className="muted">{enemies.length}/5 ENEMY</span></div>
          ...
          <div className="field"><div className="label">MY CHAMPION</div><input className="search" placeholder="Champion keresés — pl. Morgana" value={search} onChange={e=>setSearch(e.target.value)}/></div>
          <div className="champGrid">{filtered.map(c=><button key={c.id} className={'champ '+(mine.id===c.id?'active':'')} onClick={()=>setMine(c)}><div className="portrait" style={{'--c1':c.c1,'--c2':c.c2}}>{championImage(c)?<img src={championImage(c)} alt={c.name} loading="lazy"/>:<span>{initials(c.name)}</span>}</div><span>{c.name}</span></button>)}{!filtered.length&&<div className="noResults">Nincs találat: <b>{search}</b></div>}</div>

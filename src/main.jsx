...
function App(){
  ...
  useEffect(()=>{fetch('/api/riot/me').then(async r=>{const d=await r.json();setRiotMonitor(d);if(d.connected&&d.account)setProfile(p=>({...p,connected:true,riotId:d.account.gameName+'#'+d.account.tagLine,lastSync:d.monitor?.lastSync||new Date().toISOString()}))}).catch(()=>setRiotMonitor({status:'offline'}))},[])
